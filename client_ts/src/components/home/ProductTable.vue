...
export default defineComponent({
  ...
  data() {
    return {
      ...
      defaultProducts: [] as ProductInterface[],
      columns: [{ field: "name", header: "Name" }],
      rows: 5 as number, // number of rows per page
      imageUrl: null,
  ...
  methods: {
    ...
    shortenAddress(id: string): string {
      return `${id.slice(0, 5)}...${id.slice(id.length - 4)}`;
    },
    setCreatedDate(date: Date): string {
      const createdAt = new Date(date);
      const options: ToLocaleDateStringOptions = {
        day: "2-digit",
        month: "2-digit",
        year: "numeric",
        hour: "numeric",
    ...
    filterData(): ProductInterface[] | undefined {
      if (this.searchTerm !== "") {
